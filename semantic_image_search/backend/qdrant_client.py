...
class QdrantClientManager:
    ...
    Bug fix: the collection previously used a named-vector config
    (`{"default": VectorParams(...)}`), but `retriever.py` queried
    with an unnamed vector. This caused a runtime mismatch. The fix
    is to use the simple unnamed VectorParams directly.
    """
    ...
    def ensure_collection(cls):
        """Ensure Qdrant collection exists with the correct (unnamed) vector config."""
        ...
        try:
            ...
            if Config.QDRANT_COLLECTION not in existing:
                ...
                # Use simple (unnamed) VectorParams so both upsert and query_points
                # work without specifying a vector name.
