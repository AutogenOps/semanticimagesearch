...
class Config:
    ...
    if HF_API_KEY:
        log.info("HF_API_KEY loaded (hidden)")
    else:
        log.warning("HF_API_KEY missing — CLIP embeddings via HF Inference API will not work")

    HF_CLIP_MODEL_ID: str = os.getenv("HF_CLIP_MODEL_ID", "openai/clip-vit-base-patch32")
    log.info("HF_CLIP_MODEL_ID loaded", value=HF_CLIP_MODEL_ID)
...
# ------------------------------------------------------------
# Final global config success log
# ------------------------------------------------------------
config = Config()
